...
public class _37BuyAndSell_3 {
    ...
    public static int memo(int[]arr, int i, int limit, int stock, int[][][]dp){
        ...
        if(stock == 0){
            return dp[i][stock][limit] = Math.max( memo(arr,i+1, limit, 0,dp), -arr[i] + memo(arr, i+1, limit, 1,dp));
        }
        else{
            return dp[i][stock][limit]= Math.max( memo(arr, i+1, limit, 1,dp), arr[i] +memo(arr, i+1, limit-1, 0, dp));
        }
    }
...
//        prev[0][0] = prev[1][0] = 0;
    ...
    // here we are using a trick that if trans no. is even then we have to buy else we can sell.
